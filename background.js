...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  ...
  // background.js에서 elementSelectorGenerated 메시지 처리 부분 수정
  ...
  if (request.action === "elementSelectorGenerated") {
    try {
      ...
      chrome.storage.local.set(
        ...
        function () {
          if (chrome.runtime.lastError) {
            ...
            // 팝업에 즉시 저장 및 적용 로직 추가
            if (request.selectionType === "tag") {
              chrome.storage.local.get(["autoExtractTags"], function (result) {
                let currentValue = result.autoExtractTags || "";
                currentValue = currentValue.trim();
                const newSelector = request.selector.trim();

                if (currentValue) {
                  // 이미 값이 있으면 쉼표로 구분하여 추가
                  currentValue = currentValue + ", " + newSelector;
                } else {
                  // 값이 없으면 그대로 설정
                ...
                // 바로 저장
                chrome.storage.local.set({ autoExtractTags: currentValue });
              });
            } else if (request.selectionType === "container") {
              // 컨테이너 선택자 바로 저장
              chrome.storage.local.set({
                containerSelector: request.selector.trim(),
              });
            } else if (request.selectionType === "button") {
              chrome.storage.local.get(
                ["loadMoreButtonSelectors"],
                function (result) {
                  let currentValue = result.loadMoreButtonSelectors || "";
                  currentValue = currentValue.trim();
                  const newSelector = request.selector.trim();
                  ...
                  // 바로 저장
                  chrome.storage.local.set({
                    loadMoreButtonSelectors: currentValue,
                  });
                },
              );
            }
      ...
      chrome.runtime.sendMessage(request, function (response) {
        if (chrome.runtime.lastError) {
          console.log(
            "팝업이 닫혀있거나 응답하지 않음, 선택자는 스토리지에 저장됨",
          );
        } else {
  ...
  if (
    request.action === "newTextExtracted" ||
    request.action === "bulkTextExtracted" ||
    request.action === "extractionError"
  ) {
    // 메시지 중복 처리 방지 로직 추가
  ...
  try {
    ...
    return new Promise((resolve) => {
      chrome.tabs.sendMessage(tabId, { action: "ping" }, function (response) {
        if (chrome.runtime.lastError) {
          console.log(
            "Content script가 로드되지 않았습니다. 로드 시도:",
            chrome.runtime.lastError.message,
          );
